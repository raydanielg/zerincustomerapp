...
class NotificationHelper {
  static Future<void> initialize(FlutterLocalNotificationsPlugin flutterLocalNotificationsPlugin) async {
    ...
    // FIXED: Using 'settings:' named parameter for initialization
    await flutterLocalNotificationsPlugin.initialize(
      settings: initializationsSettings,
      onDidReceiveNotificationResponse: (NotificationResponse payload) async {
        // Handle notification tap
        if (payload.payload != null && payload.payload!.isNotEmpty) {
          notificationRouteCheck({'ride_request_id': payload.payload, 'action': 'notification_click'});
        }
      },
    ...
    FirebaseMessaging.onMessage.listen((RemoteMessage message) async {
      ...
      // FIXED: Using 'settings:' named parameter for foreground initialization
      await flutterLocalNotificationsPlugin.initialize(
        settings: initializationsSettings,
  ...
  static Future<void> hintForBetterServiceLocationTurnOn({String? body}) async {
    ...
    // FIXED: Using named parameters in .show() to match the modern package signature
      ...
      if(Get.find<RideController>().tripDetails!.type == AppConstants.parcel){
        if(Get.find<RideController>().tripDetails!.currentStatus == AppConstants.accepted || Get.find<RideController>().tripDetails!.currentStatus == AppConstants.ongoing){
          if(Get.currentRoute != '/MapScreen'){
            ...
            _toRoute(formSplash, const MapScreen(fromScreen: MapScreenType.parcel)),
          }
  ...
  static Future<void> _toRoute(bool formSplash, Widget page) async {
    if(formSplash) {
...
// FIXED: Defined background callback matching the documentation example
@pragma('vm:entry-point')
void notificationTapBackground(NotificationResponse notificationResponse) {
  customPrint('notification(${notificationResponse.id}) action tapped: '
